...
打开项目后，**工作台主页和所有工作页面右侧都有 Codex 聊天栏**。默认使用“项目对话 · 从选题开始”，无需先创建主题或平台版本，就能讨论选题、定位、素材、排期和复盘；切换页面或收起面板仍保留对话与未发送输入。右上角按钮可展开 / 收起。

需要继续原来的主题对话时，在“对话范围”选择对应主题；选择“起草版本”后再指定平台版本，文案会写回该版本。各项目和主题的已发送记录分别保存，重启后可继续。详见 [全流程 Codex 聊天](docs/project-chat.md)。升级此次功能后须保存并退出旧开发窗口，再运行 `npm run dev`，以加载新的项目对话接口。
