...
- V0.2 日历区分辅助、模拟和本地自动排期；SQLite 租约与提交意图保护最终提交，按账号串行、全局双并发。关机不发送，恢复备份的未发送任务暂停，已提交任务保留待核实。连接与系统加密凭据存放在应用用户数据目录，业务项目不保存密钥。具体恢复规则见 [V0.2 手册](v0.2-publishing.md)。
