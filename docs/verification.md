...
工作台主页与所有工作页面统一提供 Codex 聊天栏，空项目可直接发起项目对话；原来的主题对话及结构化起草保留。类型检查、ESLint、源码构建和全部 57 项自动化检查通过（49 项 Node 测试、8 项 Electron 桌面回归），未生成安装包。

新增覆盖空项目不创建占位主题、跨页面同一模型会话、重启续聊、项目 / 主题 / Codex 账号隔离、停止任务与只读限制；桌面模拟覆盖主页 Enter 发送、Shift+Enter 换行、输入法确认、七个工作页面与编辑页切换、收起后保留输入、持续接收回复、错误后保留输入以及 1280 / 1920 窗口布局。截图位于忽略的 `.local/e2e-evidence/global-codex-dashboard.png`。操作方式见 [全流程 Codex 聊天](project-chat.md)。

真实 Codex CLI 0.153.4 的两轮项目对话已完成：从主页切换到素材阶段后仍能回答上一轮的“雨后散步”测试选题，两轮复用同一线程，内容主题数量保持 0。验证只使用隔离项目，未操作日常业务内容；结果保存在忽略的 `.local/project-chat-verification/`。

## 2026-09-08 · 0.2.1 小红书网页登录

新增 **账号与定位 → 网页登录**，支持打开小红书官方登录页、按项目和账号保存独立会话、检查身份以及退出本机登录。操作方式见 [小红书登录说明](xiaohongshu-login.md)。本次通过类型检查、ESLint、源码构建和全部 55 项自动化测试：10 项领域与登录判定、13 项集成、23 项发布与适配器、2 项缩略图，以及 7 项真实 Electron 桌面回归。

新增桌面测试使用合成的官方页面响应，验证两个账号隔离、会话跨重启、网络异常保留会话、清除 Cookie 和 localStorage、退出时忽略旧的检查结果、外站导航拦截，以及关闭主窗口时一并退出登录窗口。远程页面无法访问工作台 IPC 或 Node 接口。

另用全新匿名 Electron 会话实际打开官网，确认短信登录表单及二维码切换入口可见、身份接口返回未登录。没有替用户扫码、发送验证码或使用真实账号完成登录，没有向平台发布内容；真实登录后的平台兼容性仍需用户完成一次登录核对。没有生成安装包。测试截图和真实页面核对结果仅保存在忽略的 `.local/` 目录。
